...
void sig(int action)
{
    h::log("Quitting due to signal [" + misc::toString(action) + "] being raised!");
    vars::logout.close();
    vars::cfgin.close();
    exit(EXIT_SUCCESS);
}
...
void setSigHandlers()
{
    ...
    {
        h::log("Signal handling disabled! Add handle_signals to the config_b <" + misc::toString(CFGB_LOCATION) + "> file.");
    }
}
...
void checkcfg()
{
    if(!vars::cfgin.is_open())
    {
        ...
        if(fout.is_open())
        {
            ...
            fout.close();
            h::log("Successfully wrote configuration file to: " + misc::toString(CFGB_LOCATION));
        ...
        {
            h::log("Unable to write configuration file to: " + misc::toString(CFGB_LOCATION) +
...
long long h::getLogSize()
{
    std::ifstream fin(LOG_LOCATION, std::ios::ate);
    if(fin.is_open())
    {
        h::log("Log size calculated to be " + misc::toString(fin.tellg()) + " bytes.");
        return fin.tellg();
    }
    h::log("Unable to open logfile to check file size!");
    return -1;
}
...
{
    vars::logout.close();
    std::ofstream fout(LOG_LOCATION);
    if(fout.is_open())
    {
        fout << "Log Cleared!" << std::endl;
        fout.close();
    }
    vars::logout.open(LOG_LOCATION, std::ios::out | std::ios::app);
    if(vars::logout.is_open())
    {
        h::log("Successfully reopened log!");
    }
    else
    {
        h::log("Unable to open log!");
    }
}
